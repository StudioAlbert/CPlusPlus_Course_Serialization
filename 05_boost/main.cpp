...
private:
    ...
    void serialize(Archive& ar, const unsigned int version) {
        ar & id_;
        ar & walkable_;
        if (version > 0) ar & biome_; // champ ajoute en v1
    }
...
int main() {
    TileMap m{2, 2, {Tile(1,true,"grass"), Tile(2,false,"rock"),
                      Tile(3,true,"sand"),  Tile(4,true,"snow")}};

    const auto path = std::filesystem::temp_directory_path() / "boost_map.txt";
    ...
    std::cout << "Recharge " << m2.tiles.size() << " tuiles ("
              << m2.w << 'x' << m2.h << "):\n";
